...
/**
 * ピクセルトラッカー設定をカスタマイザーに追加
 */
function pout_pixel_tracker_customizer($wp_customize) {
    $wp_customize->add_section('pout_pixels', array(
        'title'       => __('ピクセルトラッカー', 'pout-theme'),
        'description' => __('広告プラットフォームのトラッキングピクセルを設定', 'pout-theme'),
        'priority'    => 35,
    ));

    // Meta Pixel (Facebook)
    $wp_customize->add_setting('pout_meta_pixel_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('pout_meta_pixel_id', array(
        'label'       => __('Meta Pixel ID', 'pout-theme'),
        'description' => __('Facebook/Instagram広告用。例: 1234567890', 'pout-theme'),
        'section'     => 'pout_pixels',
        'type'        => 'text',
    ));

    // LINE Tag
    $wp_customize->add_setting('pout_line_tag_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('pout_line_tag_id', array(
        'label'       => __('LINE Tag ID', 'pout-theme'),
        'description' => __('LINE広告用。例: 12345678-abcd-1234-abcd-123456789012', 'pout-theme'),
        'section'     => 'pout_pixels',
        'type'        => 'text',
    ));

    // X (Twitter) Pixel
    $wp_customize->add_setting('pout_twitter_pixel_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('pout_twitter_pixel_id', array(
        'label'       => __('X (Twitter) Pixel ID', 'pout-theme'),
        'description' => __('X広告用。例: abcde', 'pout-theme'),
        'section'     => 'pout_pixels',
        'type'        => 'text',
    ));

    // TikTok Pixel
    $wp_customize->add_setting('pout_tiktok_pixel_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('pout_tiktok_pixel_id', array(
        'label'       => __('TikTok Pixel ID', 'pout-theme'),
        'description' => __('TikTok広告用。例: ABCDEFGH12345', 'pout-theme'),
        'section'     => 'pout_pixels',
        'type'        => 'text',
    ));

    // Pinterest Tag
    $wp_customize->add_setting('pout_pinterest_tag_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('pout_pinterest_tag_id', array(
        'label'       => __('Pinterest Tag ID', 'pout-theme'),
        'description' => __('Pinterest広告用', 'pout-theme'),
        'section'     => 'pout_pixels',
        'type'        => 'text',
    ));

    // LinkedIn Insight Tag
    $wp_customize->add_setting('pout_linkedin_partner_id', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('pout_linkedin_partner_id', array(
        'label'       => __('LinkedIn Partner ID', 'pout-theme'),
        'description' => __('LinkedIn広告用', 'pout-theme'),
        'section'     => 'pout_pixels',
        'type'        => 'text',
    ));
}
add_action('customize_register', 'pout_pixel_tracker_customizer');

/**
 * Meta Pixel (Facebook/Instagram) 出力
 */
function pout_output_meta_pixel() {
    $pixel_id = get_theme_mod('pout_meta_pixel_id', '');
...
add_action('wp_head', 'pout_output_meta_pixel', 3);

/**
 * LINE Tag 出力
 */
function pout_output_line_tag() {
    $tag_id = get_theme_mod('pout_line_tag_id', '');
    ...
    (function(g,d,o){
      ...
      s.src="https://d.line-scdn.net/n/line_tag/public/release/v1/lt.js";h.parentNode.insertBefore(s,h);
    })(window,document,'script');
    _lt('init', {
      customerType: 'lap',
      tagId: '<?php echo esc_js($tag_id); ?>'
    });
    _lt('send', 'pv', ['<?php echo esc_js($tag_id); ?>']);
    </script>
    <noscript>
    <img height="1" width="1" style="display:none"
    src="https://tr.line.me/tag.gif?c_t=lap&t_id=<?php echo esc_attr($tag_id); ?>&e=pv&noscript=1" />
    </noscript>
    <!-- End LINE Tag -->
    <?php
}
add_action('wp_head', 'pout_output_line_tag', 3);

/**
 * X (Twitter) Pixel 出力
 */
function pout_output_twitter_pixel() {
    $pixel_id = get_theme_mod('pout_twitter_pixel_id', '');
    ...
    twq('config','<?php echo esc_js($pixel_id); ?>');
    </script>
    <!-- End X Pixel Code -->
    <?php
}
add_action('wp_head', 'pout_output_twitter_pixel', 3);

/**
 * TikTok Pixel 出力
 */
function pout_output_tiktok_pixel() {
    $pixel_id = get_theme_mod('pout_tiktok_pixel_id', '');
...
add_action('wp_head', 'pout_output_tiktok_pixel', 3);

/**
 * Pinterest Tag 出力
 */
function pout_output_pinterest_tag() {
    $tag_id = get_theme_mod('pout_pinterest_tag_id', '');
    ...
    pintrk('load', '<?php echo esc_js($tag_id); ?>');
    pintrk('page');
    </script>
    <noscript>
    <img height="1" width="1" style="display:none;" alt=""
      src="https://ct.pinterest.com/v3/?event=init&tid=<?php echo esc_attr($tag_id); ?>&noscript=1" />
    </noscript>
    <!-- End Pinterest Tag -->
    <?php
}
add_action('wp_head', 'pout_output_pinterest_tag', 3);

/**
 * LinkedIn Insight Tag 出力
 */
function pout_output_linkedin_insight() {
    $partner_id = get_theme_mod('pout_linkedin_partner_id', '');
    ...
    <noscript>
    <img height="1" width="1" style="display:none;" alt=""
    src="https://px.ads.linkedin.com/collect/?pid=<?php echo esc_attr($partner_id); ?>&fmt=gif" />
    </noscript>
    <!-- End LinkedIn Insight Tag -->
    <?php
}
add_action('wp_head', 'pout_output_linkedin_insight', 3);

/**
 * コンバージョンイベント用ヘルパー関数
 */
function pout_track_conversion($event_name, $params = array()) {
    $output = '';

    // Meta Pixel
    if (get_theme_mod('pout_meta_pixel_id', '')) {
        $params_json = !empty($params) ? ', ' . wp_json_encode($params) : '';
        $output .= "<script>fbq('track', '" . esc_js($event_name) . "'" . $params_json . ");</script>\n";
    }

    // LINE Tag
    if (get_theme_mod('pout_line_tag_id', '')) {
        $tag_id = get_theme_mod('pout_line_tag_id', '');
        $output .= "<script>_lt('send', 'cv', {type: '" . esc_js($event_name) . "'}, ['" . esc_js($tag_id) . "']);</script>\n";
    }

    // TikTok
    if (get_theme_mod('pout_tiktok_pixel_id', '')) {
        $params_json = !empty($params) ? ', ' . wp_json_encode($params) : '';
        $output .= "<script>ttq.track('" . esc_js($event_name) . "'" . $params_json . ");</script>\n";
    }
...
/**
 * お問い合わせフォーム送信時のコンバージョントラッキング
 */
function pout_contact_form_conversion() {
    if (!isset($_GET['contact']) || $_GET['contact'] !== 'success') {
        return;
    }

    echo pout_track_conversion('Lead');
}
add_action('wp_footer', 'pout_contact_form_conversion');
